...
public final class FoxAlgorithm implements IAlgorithm {
    ...
    public Result solve(int threadsNumber) {
        ...
        var sSplitted = Matrix.splitMatrix(this.secondMatrix, subMatrixNum);
        
        var matrixLength = fSplitted.length;
        var subMatrixSize = fSplitted[0][0].getRows();
        ...
        for (var k = 0; k < matrixLength; k++) {
            ...
            for (var i = 0; i < matrixLength; i++) {
                ...
                for (var j = 0; j < matrixLength; j++) {
                    tasks.add(threadPool.submit(new FoxAlgorithmThread(fSplitted[i][offset], sSplitted[offset][j], resultMatrix[i][j])));
                }
            }
